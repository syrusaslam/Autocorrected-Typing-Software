...
def accuracy(typed, source):
    ...
    for x in range(min(len(source_words), len(typed_words))):
        if typed_words[x] == source_words[x]:
            total += 1
    return float(total / len(typed_words)) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    chosen_word = min(word_list, key = lambda closest: diff_function(typed_word, closest, limit))
    if diff_function(typed_word, chosen_word, limit) <= limit:
        return chosen_word
    else:
        return typed_word
...
def report_progress(typed, source, user_id, upload):
    ...
    for x in range(len(typed)):
        if typed[x] == source[x]:
            ...
            break
    upload({'id': user_id, 'progress': total / len(source)})
...
def time_per_word(words, timestamps_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    timestamp = [[j[i+1] - j[i] for i in range(len(j) - 1)] for j in timestamps_per_player]
    return match(words, timestamp)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    temp = [[] for i  in player_indices]
    for x in word_indices:
        player_times = [time(match, y, x) for y in player_indices]
        temp[player_times.index(min(player_times))].append(get_word(match, x))
    return temp
    # END PROBLEM 10
